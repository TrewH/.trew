...
install_fastfetch() {
  ...
  # Try apt only if the repo provides it (Ubuntu 24.04+, not 20.04)
  if command -v apt >/dev/null 2>&1; then
    if dpkg --compare-versions "$(lsb_release -rs 2>/dev/null || echo 0)" ge 24.04; then
      sudo apt update && sudo apt install -y fastfetch && return 0
    fi
  fi

  # Build from source (works on 20.04)
  ...
  cmake -S "$tmpdir/fastfetch" -B "$tmpdir/fastfetch/build" -DCMAKE_BUILD_TYPE=Release
  cmake --build "$tmpdir/fastfetch/build" -j"$(nproc)"
  sudo cmake --install "$tmpdir/fastfetch/build"   # installs /usr/local/bin/fastfetch

  # Ensure PATH includes /usr/local/bin for this session
  export PATH="/usr/local/bin:$PATH"

  if ! command -v fastfetch >/dev/null 2>&1; then
    echo "fastfetch install failed; as a fallback, you can: sudo apt install -y neofetch"
    return 1
  fi
}
...
install_font() {
    ...
    if fc-list :family | grep -iq "$FONT_NAME"; then
        ...
        if grep -qi microsoft /proc/version 2>/dev/null && [ -z "${WAYLAND_DISPLAY:-}${DISPLAY:-}" ]; then
            echo "[.trew] Detected WSL without GUI. Install a Nerd Font on WINDOWS and select it in Windows Terminal > Profile > Appearance."
        fi
